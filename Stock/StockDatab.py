...
StockCode = ["600198","300033","300368","300307","600570","002368","600389","002402","002655","600547","601311","000858","300236","002460","000728","002746","300315","600150","603885","002048","600120","300093","300242","300247","002047","600503"]#"000538",
...
def CalculateInfo():
	...
	for key,value in enumerate(NowStocksInfo):
		...
		info += CalculateBigFish(value)   	  		#(1)统计主力流入情况		√
	 	info += CalculatePrice(value)		  		#(2)统计价格				√
		info += CaculateKDJ(value)		  	  		#(3)统计JDK				√
		info += CaculateMACD(value)		  	  		#(4)统计MACD日线			√
		info += CaculateRSI(value)			  		#(5)统计RSI				√
		info += CaculateBoll(value)			  		#(6)统计Boll通道			√
		info += CalculateJump(value)			  	#(7)统计跳空				√
		info += CalculateVolume(value)		  		#(8)成交量对比			√
		info += CalculateMovingAverage(value)  		#(9)日线支撑压迫			√
		info += "\n\n"
